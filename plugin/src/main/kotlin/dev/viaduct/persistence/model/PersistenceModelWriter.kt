        ...
        destination.writeText(
            buildString {
                ...
                model.entities.sortedBy(PersistenceEntity::graphqlName).forEach { entity ->
                    appendLine(
                        listOf("entity", entity.graphqlName, entity.generatedGlobalId.toString())
                            .joinToString("\t"),
                    )
                    entity.attributes.forEach { attribute -> appendLine(attributeRow(entity, attribute)) }
                }
                model.enums.sortedBy(PersistenceEnum::graphqlName).forEach { enumValue ->
                    appendLine(
                        (listOf("enum", enumValue.graphqlName) + enumValue.values)
                            .joinToString("\t"),
                    )
                }
            },
