...
internal object HibernateReferenceManifestReader {
    fun read(source: File): HibernateReferenceManifest {
        ...
        val allowed = setOf(
            "mapping-file", "classpath", "managed-class", "implicit-naming-strategy",
            "physical-naming-strategy", "metadata-customizer", "dialect", "setting",
            "ownership-manifest",
        )
        ...
        fun repeated(name: String): List<String> = records
            .filter { it.firstOrNull() == name }
            .map { record ->
                require(record.size == 2) { "Invalid $name record in ${source.absolutePath}" }
                record[1]
            }
        val settings = records.filter { it.firstOrNull() == "setting" }.associate { record ->
            require(record.size == 3) { "Invalid setting record in ${source.absolutePath}" }
