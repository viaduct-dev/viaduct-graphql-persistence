...
class CollectionRelationshipTest {
    ...
    fun `uses join tables for duplicate and mutual collection relationships`() {
        ...
        val primary = alpha.attributes.single { it.name == "primaryBetas" }
            as PersistenceToManyAttribute
        val featured = alpha.attributes.single { it.name == "featuredBetas" }
            as PersistenceToManyAttribute
        ...
        val alphaGammas = alpha.attributes.single { it.name == "gammas" }
            as PersistenceToManyAttribute
        val gammaAlphas = model.entities.single { it.graphqlName == "Gamma" }
            .attributes.single { it.name == "alphas" } as PersistenceToManyAttribute
        assertEquals(
            setOf(
    ...
    fun `uses a join table for a self-referential collection`() {
        ...
        val friends = model.entities.single()
            .attributes.single { it.name == "friends" } as PersistenceToManyAttribute
